...
import { parseAPI, predictAPI, modelsAPI, getErrorMessage } from '@/lib/api'
import { extractCategoricalValues, type CategoricalValues } from '@/lib/metadata-parser'
...
export function PredictPage() {
  ...
  const [categoricalValues, setCategoricalValues] = useState<CategoricalValues>({
    directions: [],
    balconyDirections: [],
    legalStatuses: [],
    districts: [],
  ...
  useEffect(() => {
    const fetchMetadata = async () => {
      try {
        const metadata = await modelsAPI.getModelMetadata()
        if (metadata.feature_names) {
          const values = extractCategoricalValues(metadata.feature_names)
          setCategoricalValues(values)
        }
      } catch (error) {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      if (response.success && response.features) {
        ...
        setShowForm(true)
        // Auto-trigger prediction after successful parse
        await autoPredictAfterParse(response.features)
      } else {
    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-6">
          ...
          <Tabs defaultValue="text-parse" value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          {/* Feature Form Card - Show after parse */}
          <AnimatePresence>
            {showForm && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-2xl flex items-center justify-center">
                          <Edit3 className="w-5 h-5 text-primary stroke-[2.5]" />
                        </div>
                        <div>
                          <CardTitle className="text-lg">Thông tin nhà</CardTitle>
                          <CardDescription className="text-xs">
                            Kiểm tra và chỉnh sửa
                          </CardDescription>
                        </div>
                      </div>
                      <Badge variant="secondary" className="rounded-lg px-2 py-0.5 text-xs font-semibold">
                        AI
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-4 pt-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {formFields.map((field) => {
                        const isSelect = ['Direction', 'BalconyDirection', 'LegalStatus', 'District', 'Furniture'].includes(field.key)
                        const getOptions = () => {
                          switch (field.key) {
                            case 'Direction':
                              return categoricalValues.directions
                            case 'BalconyDirection':
                              return categoricalValues.balconyDirections
                            case 'LegalStatus':
                              return categoricalValues.legalStatuses
                            case 'District':
                              return categoricalValues.districts
                            case 'Furniture':
                              return categoricalValues.furniture
                            default:
                              return []
                          }
                        }
                    ...
                    <div className="flex items-center gap-3 pt-3 border-t">
                      ...
                      />
                      <Label htmlFor="ensemble" className="cursor-pointer text-xs font-medium">
                        Sử dụng Ensemble (nhiều mô hình)
                      </Label>
                    </div>
                    ...
                    <div className="flex gap-2 pt-3">
                      <Button
                        ...
                        onClick={handleReset}
                        disabled={predictLoading}
                        className="flex-1 rounded-lg border-2 font-semibold h-10 text-sm"
                      >
                      ...
                      <Button
                        ...
                        className="flex-1 rounded-lg bg-gradient-to-r from-primary to-primary/80 hover:shadow-lg hover:shadow-primary/30 transition-all duration-300 font-semibold h-10 text-sm"
                      >
                        {predictLoading ? (
                          <>
                            <span className="animate-spin mr-2">⏳</span>
                ...
                <Card className="w-full min-h-[500px] flex items-center justify-center border shadow-md rounded-3xl">
                  <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="text-muted-foreground text-base max-w-md leading-relaxed">
                      Nhập mô tả nhà và nhấn "Dự đoán giá" để bắt đầu
                    </p>
                  </CardContent>
                </Card>
              </motion.div>
