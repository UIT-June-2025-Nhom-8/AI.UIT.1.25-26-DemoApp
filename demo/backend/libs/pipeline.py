...
class MLPipeline:
    ...
    def train_model(self, model_name: str, tune_hyperparameters: bool = False):
        ...
        scoring = self.config['training'].get('scoring', 'r2')
        ...
        print(f"CV {scoring}: {cv_score['mean']:.4f} ± {cv_score['std']:.4f}")
    ...
    def run_pipeline(self,
        ...
        test_path = self.config['dataset'].get('test_path')

        if test_path:
            # Load pre-split train and test data
            print("Loading pre-split train and test datasets...")
            X_train, y_train = self.load_data(data_path or self.config['dataset']['train_path'])
